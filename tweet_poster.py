...
class TweetPoster:
    ...
    def _post_with_retry(self, article: Dict[str, Any], max_retries: int = BotConstants.MAX_RETRIES) -> Optional[str]:
        ...
        for attempt in range(max_retries + 1):
            try:
                # Create the tweets that will make up the thread (2-part format for backward compatibility)
                ...
                # Post reply tweet if link text is available
                if link_text and link_text.strip():
                    try:
                        ...
                        if self._looks_like_rate_limit_response(reply):
                            logger.warning("Reply tweet response resembles rate limit; skipping reply.")
                        else:
